...
$page_title      = "Modifier un verre";
$page_breadcrumb = "Stock / Verres / <span>Modifier</span>";

$erreur = "";
$succes = "";

$id  = isset($_GET['id']) ? intval($_GET['id']) : 0;
$res = mysqli_query($con, "SELECT * FROM verre WHERE id = $id");
if (!$res || mysqli_num_rows($res) === 0) {
    header("Location: verre_list.php");
    exit();
}
$verre = mysqli_fetch_assoc($res);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ref === '' || $prix === '') {
        ...
        mysqli_query($con, "UPDATE verre SET
            ref_verre='$ref_s', prix_verre='$prix_s', stock=$stock,
            id_categorie=$id_cat, id_marque=$id_marque
            WHERE id=$id");
        $succes = "Verre modifié avec succès.";
        $verre = array_merge($verre, array(
            'ref_verre'    => $ref,
            'prix_verre'   => $prix,
            'stock'        => $stock,
            'id_categorie' => $id_cat,
...
<div style="max-width:600px;">
    <div class="card-dark">
        <div class="card-header"><i class="bi bi-pencil me-2"></i>Modifier le verre</div>
                ...
                <div class="mb-3">
                    ...
                    <select name="id_categorie" class="form-select">
                        ...
                        <option value="<?php echo $c['id']; ?>"
                            <?php echo $c['id'] == $verre['id_categorie'] ? 'selected' : ''; ?>>
                ...
                <div class="mb-3">
                    ...
                    <select name="id_marque" class="form-select">
                        ...
                        <option value="<?php echo $m['id']; ?>"
                            <?php echo $m['id'] == $verre['id_marque'] ? 'selected' : ''; ?>>
